...
from gestor import ware_gestor, customer_gestor, sales_gestor
...
class sales_details(QtWidgets.QDialog):
    ...
    def init_condition(self):
        self.addFlag = False #esto es necesario para ver si se puede cargar la ventana anterior
        self.comparativeList = None
        # -----------  set item conditions  -----------
        self.cantCash = 0.0
        self.cantCredit = 0.0
        self.cantItems = 0
    ...
    def GuardarButton(self, event):
        doc = self.txtCliente.text().split(" ")[0]
        index_ = next((index for (index, d) in enumerate(self.custGest.Customers) if d.doc == doc), None)

        if index_ != None and self.txtCliente.text() != "" and self.custGest.Customers[index_].id != 7 and len(self.main_table) > 0:

            text_, validation_ = QtWidgets.QInputDialog.getText(self, 'Validar operación',
                                                                "Ingrese contraseña para guardar venta",
                                                                QtWidgets.QLineEdit.Password)
            index = next((index for (index, d) in enumerate(self.ownUsers[1]) if d.passwd == text_), None)
            if index != None and validation_:
                if self.salesGest.addSale(self.DateSales, self.main_table, self.ownWares[0], self.custGest.Customers[index_].id, self.ownUsers[1][index].user, True):
                    QMessageBox.question(self, 'Alerta', "Venta guardada", QMessageBox.Ok, QMessageBox.Ok)
                    self.addFlag = True
                    self.widget.setCurrentIndex(self.widget.currentIndex() - 1)

        #estas dos ultimas condiciones son las que van con 0

        elif index_ != None and self.txtCliente.text() != "" and self.custGest.Customers[index_].id == 7 and len(self.main_table) > 0:
            text_, validation_ = QtWidgets.QInputDialog.getText(self, 'Validar operación',
                                                                "Ingrese contraseña para guardar venta",
                                                                QtWidgets.QLineEdit.Password)
            index = next((index for (index, d) in enumerate(self.ownUsers[1]) if d.passwd == text_), None)
            ...
            if index != None and validation_:
                self.changeUnknownItems()
                self.salesGest.addSale(self.DateSales, self.main_table, self.ownWares[0],
                                       self.custGest.Customers[index_].id, self.ownUsers[1][index].user, False)
                QMessageBox.question(self, 'Alerta', "Venta guardada", QMessageBox.Ok, QMessageBox.Ok)
                self.addFlag = True
                self.widget.setCurrentIndex(self.widget.currentIndex() - 1)

        elif self.txtCliente.text() == "" and len(self.main_table) > 0:
            reply = QMessageBox.question(self, 'Window Close', 'Desea guardar la venta como cliente desconocido?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                ...
                if index != None and validation_:
                    self.changeUnknownItems()
                    self.salesGest.addSale(self.DateSales, self.main_table, self.ownWares[0], 7, self.ownUsers[1][index].user, False)
                    QMessageBox.question(self, 'Alerta', "Venta guardada", QMessageBox.Ok, QMessageBox.Ok)
                    self.addFlag = True
                    self.widget.setCurrentIndex(self.widget.currentIndex() - 1)

        elif len(self.main_table) == 0:
            QMessageBox.information(self, 'Aviso', "No hay items agregados")

    def newUserButton(self, event):
        self.widget.setCurrentIndex(self.widget.currentIndex() + 1)

    def changeUnknownItems(self):
        for j in self.main_table:
            for i in self.comparativeList:
                if j["cod"] == i[1] and j["tarjeta"] == i[2]:
                    j["id"] = i[0]
    ...
    def setupUi(self):
        ...
        self.gbCustomer.setGeometry(QtCore.QRect(410, 0, 300, 60))
        ...
        # -----------  NewUser cofiguration  -----------
        self.newUser_label = QtWidgets.QLabel(self.gbCustomer)
        self.newUser_label.setGeometry(QtCore.QRect(250, 22, 30, 30))
        self.newUser_label.setText("")
        self.newUser_label.setPixmap(QtGui.QPixmap("C:/Users/IROJAS/PycharmProjects/genesis/imgs/new_customer.png"))
        self.newUser_label.setScaledContents(True)
        self.newUser_label.setObjectName("newUser_label")
        self.newUser_label.mousePressEvent = self.newUserButton
        ...
        self.btnGuardar.clicked.connect(self.GuardarButton)
        # self.btnGuardar.mousePressEvent = self.GuardarButton
        ...
        self.btnregresar.clicked.connect(self.regresarButtonEvent)
